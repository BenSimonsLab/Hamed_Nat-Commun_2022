...
files_processed = fnmatch.filter(os.listdir('data/scrublet-scores'), "HAM*.csv")
sampleIDs_processed = [re.sub('\.csv$', '', sample) for sample in files_processed]
sampleIDs = os.listdir('data/samples')
...
for sample in [i for i in sample_list if os.path.isdir('data/samples/'+i)]:
    #import data
    adata_sample = sc.read_10x_h5('data/samples/'+sample+'/outs/filtered_feature_bc_matrix.h5')
...
sample_filenames = [i for i in glob.glob('data/scrublet-scores/HAM*.{}'.format('csv'))]
